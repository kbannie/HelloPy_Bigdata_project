...
def map_nowon(geo_data, dong_data):
    ...
    fmap = folium.Choropleth(
        geo_data=geo_data,
        data=dong_data,
        columns=['Code','Scaled'],  # Modify these columns accordingly
        fill_color='YlGnBu',
...
def main():
    ...
    dong_data = pd.read_csv('./data/mp22_map.csv', encoding='cp949' )
